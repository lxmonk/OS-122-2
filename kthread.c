...
int sys_kthread_mutex_lock( int mutex_id ) {
    ...
    if (mutex->count == 0) {
        mutex->waiting_kthreads[mutex->first] = get_id();
        mutex->count++;
        release(&(mutex->lock));
        return 0;
    }
    //join the waiting kthread
    mutex->waiting_kthreads[(mutex->first + mutex->count) % NPROC] = get_id();
    mutex->count++;
...
int sys_kthread_cond_wait(int cond_id, int mutex_id) {
    ...
    sys_kthread_mutex_unlock(mutex_id);
    kthread_block(get_id());
    sys_kthread_mutex_lock(mutex_id);
    return 0;
}
